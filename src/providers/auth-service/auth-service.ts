...
export class AuthServiceProvider {
  ...
  public login(credentials) {
    if (credentials.email == null || credentials.password == null) {
      ...
      return Observable.create(observer => {
        ...
        this.http.get('http://104.248.185.208:8080/checkValidity?email=' + credentials.email + '&&password=' + credentials.password).map(res => res.json()).subscribe(data => {
          console.log(data);
  ...
  public update(){
    return Observable.create( observer=>{
      ...
      this.http.get('http://104.248.185.208:8080/updatePassenger?email=' + credential.email, { headers: headers })
        .map(res => res.json())
        .subscribe(data => {
